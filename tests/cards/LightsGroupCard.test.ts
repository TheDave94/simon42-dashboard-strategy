...
function mount(): LightsGroupCardEl {
  return document.createElement('simon42-lights-group-card') as LightsGroupCardEl;
}

describe('simon42-lights-group-card', () => {
  describe('setConfig', () => {
  ...
  describe('picker integration', () => {
    it('registers with window.customCards including preview:true', () => {
      const entry = (window.customCards || []).find(
        (c: { type: string }) => c.type === 'simon42-lights-group-card',
    ...
    it('getStubConfig returns group_type=all', () => {
      const ctor = customElements.get('simon42-lights-group-card') as
